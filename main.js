...
function getAppStorage(){
  return JSON.parse(localStorage.getItem("sakthi-notes") || "[]");
}
...
function saveNotes(notes){
  localStorage.setItem("sakthi-notes",JSON.stringify(notes));
}
...
function deleteNotes(id,textElement){
    const notes=getAppStorage();
    const updateElement=notes.filter((note)=>note.id!=id);
    saveNotes(notes);
